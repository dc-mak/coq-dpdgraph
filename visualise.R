...
sample_nodes <- nodes[sample(nrow(nodes), 500),]
...
module_nodes <- cypher(graph, "
  ...
  RETURN m.objectId AS id,
         ...
         m.path AS title,
         \"circle\" AS shape")
module_nodes$value <- median(sample_nodes$value)
...
visNetwork(rbind(sample_nodes, module_nodes),
           rbind(edges, module_edges, parent_edges),
           width="100%") %>%
  visPhysics(stabilization=FALSE) %>%
  visEdges(smooth=FALSE) %>%
  visSave(file = "modules_modularity3.html")
time <- toc(quiet=TRUE); time <- time$toc - time$tic
cat(sprintf("done. (%.2fs)\n", time))

# Ouput graph
cat("Outputting modules_modularity.html... "); tic()
visNetwork(sample_nodes, edges, width="100%") %>%
  ...
  visPhysics(stabilization=FALSE) %>%
  visEdges(smooth=FALSE) %>%
  visSave(file = "modules_modularity4.html")
time <- toc(quiet=TRUE); time <- time$toc - time$tic
cat(sprintf("done. (%.2fs)\n", time))
